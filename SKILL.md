...
| G1 | **Phase 1 预检** — 必须用 `gate-phase1.py`，检查 PRODUCT.md 存在性+US-*/INV-*/优先级关键词+conversation.md | 脚本 PASS/FAIL + 码生成 |
| G2 | **Phase 2 预检** — 必须用 `gate-phase2.py`，检查 TECH.md 存在性+架构关键词+行数 | 脚本 PASS/FAIL + 码生成 |
| G3 | **Phase 3 预检** — 必须用 `gate-phase3.py`，检查 proposal.md + constitution.md 存在性+关键词 | 脚本 PASS/FAIL + 码生成 |
| G4 | **Phase 4 预检** — 必须用 `gate-phase4.py`（内嵌码生成），检查 6 文件存在性+关键词+行数 | 脚本 PASS/FAIL + 码生成（原子化） |
| G5 | **机械确认码** — 必须用门禁脚本生成码（sha256(project+phase+time+salt)），带 TTL 过期（30 分钟），禁止手动生成 | 脚本输出（不可伪造） |
...
|------|------|------------------|---------|
|| **Tool Interception** | 工具层面阻断，不靠 LLM 自觉 | G2 gate-phase2.py、G3 gate-phase3.py、G4 gate-phase4.py、G7 gate-phase7.py | Phase 1 仍依赖人工确认（需求澄清阶段难以机械化） |
...
**追问停止条件：** 满足以下任一条件时停止追问，进入文档生成：
1. 连续 2 个维度的 L0 追问未发现新信息
2. 用户明确表示"就这些"或"没有更多了"
3. 已完成 3 个维度的 L1 追问
...
## Task N: 标题
- **负责人角色**: coder
- **需要技能**: test-driven-development, incremental-implementation
```
...
| 门禁 | 检查内容 | 未通过 → |
|------|---------|---------|
| Phase 1 | gate-phase1.py（PRODUCT.md 存在性+US-*/INV-*/优先级+conversation.md）→ 用户确认码 | 不进入 Phase 2 |
| Phase 2 | gate-phase2.py（TECH.md 存在性+关键词+行数）→ 用户确认码 | 不进入 Phase 3 |
| Phase 3 | gate-phase3.py（proposal.md + constitution.md 存在性+关键词）→ 用户确认码 | 不进入 Phase 4 |
| Phase 4 | gate-phase4.py（6 文件存在性+关键词+行数）→ 用户确认码 | 不进入 Phase 5 |
...
| **Gate 加固记录** | `references/gate-script-hardening-20260621.md` — 2026-06-21 安全加固（HMAC/熵/关键词/severity） |
| **审计方法论** | `references/gate-script-audit-methodology.md` — 4 轮模型循环审查模式、Critical 降级模式、高频发现清单 |
| **v6.2→v6.3 审计记录** | `references/audit-record-v6.2-to-v6.3.md` — 10 项修复 + 降级案例 + 评分变化 |
| **mimo 对比分析** | `references/mimo-branch-analysis-report.md` — mimo 分支对比 + 9 轮 review 结论 |
...
| v6.2.0 | 2026-06-22 | **Phase 1-3 gate 脚本全覆盖**：新增 gate-phase1.py（PRODUCT.md+conversation.md 检查）、gate-phase2.py（TECH.md 检查）和 gate-phase3.py（proposal.md+constitution.md 检查），消除 Phase 1-3 安全盲区；Layer 1 Gate 规则重新编号（G1→Phase1, G2→Phase2, G3→Phase3, G4→Phase4, G5→确认码）；流程门禁总览表逐 Phase 拆分；新增 proposal-md-template.md 模板；phase-confirmations.md 重写对齐 Phase 0-8；tasks-template.md 里程碑扩展至 Phase 0-8；gate_utils.py 新增 GATE_SECRET、TTL、verify_marker_with_expiry()；gate-phase2/3 添加目录存在性检查；gate-phase5 支持 ##/### 标题 + 中文技能 regex；SKILL.md 追问停止条件 |
