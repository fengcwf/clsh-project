...
| C8 | **kanban 派发必须注入 skills** — 每个 kanban 卡的 `--skill` 参数必须包含角色默认技能。不指定 = 流程违规。映射：coder→test-driven-development+incremental-implementation, artist→popular-web-designs+frontend-ui-engineering, tester→code-review-and-quality+systematic-debugging | `gate-phase5.py`（tasks.md skills 字段检查） |
