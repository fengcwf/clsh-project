...
**已有：**
- `/cp` = `/clsh-project` 的别名（SKILL.md frontmatter name: clsh-project → 注册 /clsh-project）
- `/cp 继续 <项目名>` = 从上次中断的 Phase 继续
- `/cp 归档 <项目名>` = Phase 7 归档

**缺口：** 这已经基本实现了 OpenSpec 的三命令极简（propose → apply → archive）。差距在于：
1. `/cp` 触发完整流程，用户可能只想快速查看进度
2. 无 `/cp 状态` 命令（查看当前 Phase + 下一步建议）
...
| 命令 | 功能 | 内部行为 |
|------|------|---------|
| `/cp` | 开始新项目 | 触发 Phase 0 → Phase 1 |
| `/cp 继续 <项目名>` | 续做 | 读 overview.md → 从当前 Phase 继续 |
| `/cp 状态 <项目名>` | 查看进度 | 读 overview.md + changes/ → 输出当前 Phase + 下一步建议 + 卡住的 blocker |
| `/cp 归档 <项目名>` | 收尾 | 触发 Phase 7 |
```

### 5.3 投入产出比

**Effort：** 极低 — 只需在 SKILL.md 的触发条件中增加一个 case
**Impact：** 低 — `/cp 继续` 已经覆盖了主要场景，`/cp 状态` 是锦上添花
**优先级：** P3 — 有空再做
