...
## ⛔ Common Pitfalls（Phase 7 高频）

> 从 pitfalls/common.md 提取的 Phase 7 高频教训，避免额外查文件。

### #10 Phase 7 归档不完整

**规则：** 归档必须检查所有必需文件（overview.md/completion-summary/retrospective/handoff.md + 每轮 Phase 8 的 conversation/diagnosis/fixes/test-report）。

**反例：** 归档时漏了 round3 的 feedback 目录 → 后续排查无记录。

### #31 调研报告写入 raw/projects/

**规则：** 分析报告 → `wiki/syntheses/`。只有有 Phase 流程的项目才进 `raw/projects/`。

**反例：** 调研报告错误写入 raw/projects/ 目录，混淆了项目文档和调研成果。

### #68 知识复利的关键是注入不是记录

**规则：** Phase 7 蒸馏后必须做教训回流（更新 SKILL.md 或 pitfalls），不只是记录 learnings.md。设计重心是"注入机制"（怎么让下次自动读到），不是"记录机制"（写到文件就完了）。

### #37 Context File Pattern 执行残留

**规则：** kanban task 的 bugfix spec 文件写入到 `raw/projects/<项目名>/changes/<变更名>/`，不是项目根目录。
