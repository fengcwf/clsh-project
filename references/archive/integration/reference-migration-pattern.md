...
│
├─ 它描述的是"某个项目的具体技术问题"？
│   → 迁移到 raw/projects/<项目名>/references/
...
```bash
# 项目专属
mkdir -p raw/projects/<项目名>/references/{integration,pitfalls}
# 跨项目共享
...
```bash
# 确认迁移文件在新位置存在
ls -la raw/projects/<项目名>/references/
# 确认 skill 目录不再有已迁移文件
...
```
执行项目 A 时：
  读 raw/projects/A/.references-meta.json
